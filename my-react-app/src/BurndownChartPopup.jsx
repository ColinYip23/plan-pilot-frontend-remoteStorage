...
function BurndownChartPopup({ sprint, tasks, onClose }) {
  ...
  dateStrings.forEach((dateStr) => {
    ...
    cumulativeCompletedStoryPointsByDate[dateStr] =
      cumulativeCompletedStoryPoints;
  });
  ...
  // Determine the actual end date for the 'Actual' line
  const actualEndDate = today < endDate ? today : endDate;

  // Filter dates up to the actual end date
  const actualDateArray = dateArray.filter((date) => date <= actualEndDate);

  // Build actual remaining story points data
  const actualDataPoints = actualDateArray.map((date) => {
    const dateStr = date.toISOString().split('T')[0];
