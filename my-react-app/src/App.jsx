...
const remoteStorage = new RemoteStorage({
  userId: '28b7c4f9-8e3e-4d4a-973e-9e2170ebd847',
  instanceId: 'my-app-instance'
});

function App() {

  //fetch data from remoteStorage
  const fetchRemoteData = async (key, defaultValue) => {
    try {
      const data = await remoteStorage.getItem(key);
      console.log(`Fetched ${key} from remote storage`);
      console.log(data);
  ...
  //save data to remoteStorage
  const saveToRemoteStorage = async (key, data) => {
    try {
      await remoteStorage.setItem(key, JSON.stringify(data));
      console.log(`Saved ${key} to remote storage`);
    } catch (error) {
      console.error(error);
    }
  }
  ...
  useEffect(() => {
    const loadUsers = async () => {
      ...
      const fetchedUsers = await fetchRemoteData('users', defaultUsers);

      // If there are no users in remoteStorage, set default user
      if (fetchedUsers.length === 0) {
        setUsers(defaultUsers);
        await remoteStorage.setItem('users', JSON.stringify(defaultUsers)); // Save default user to remoteStorage
      } else {
  ...
  // Save users to remoteStorage when they change
  useEffect(() => {
    saveToRemoteStorage('users', users);
  }, [users]);
  ...
  useEffect(() => {
    const loadCurrentUser = async () => {
      const fetchedCurrentUser = await fetchRemoteData('currentUser', null);
  ...
  useEffect(() => {
    saveToRemoteStorage('currentUser', currentUser);
  }, [currentUser]);
  ...
  useEffect(() => {
    const loadLoginState = async () => {
      const fetchedLoginState = await fetchRemoteData('isLoggedIn', false);
      setIsLoggedIn(fetchedLoginState);
    };
    loadLoginState();
  }, []);

  useEffect(() => {
    saveToRemoteStorage('isLoggedIn', isLoggedIn);
  }, [isLoggedIn]);
  ...
  const handleLogin = (username, password) => {
    ...
    if (user) {
      setCurrentUser(user);
      setIsLoggedIn(true);
      saveToRemoteStorage('isLoggedIn', true);
      saveToRemoteStorage('currentUser', user);
      setLoginError('');
    } else {
  ...
  const handleLogout = () => {
    setIsLoggedIn(false);
    setCurrentUser(null);
    saveToRemoteStorage('isLoggedIn', false);
  ...
  useEffect(() => {
    const loadTasks = async () => {
      const fetchedTasks = await fetchRemoteData('tasks', []);
  ...
  useEffect(() => {
    saveToRemoteStorage('tasks', tasks);
  }, [tasks]);
  ...
  useEffect(() => {
    const loadSprints = async () => {
      const fetchedSprints = await fetchRemoteData('sprints', []);
  ...
  useEffect(() => {
    saveToRemoteStorage('sprints', sprints);
  }, [sprints]);
